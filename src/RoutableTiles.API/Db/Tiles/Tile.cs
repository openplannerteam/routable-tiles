namespace RoutableTiles.API.Db.Tiles;
...
public class Tile
{
    ...
    {
        Tile.CreateAroundLocationRobust(lat, lon, zoom, out var x, out var y);
    ...
    /// <summary>
    /// A fast method of calculating x-y without creating a tile object and limits both lat and lon to acceptable ranges.
    /// </summary>
    public static void CreateAroundLocationRobust(double lat, double lon, int zoom, out int x, out int y)
    {
        if (lon < -180) lon = -180;
        if (lon > 180) lon = 180;
        ...
        CreateAroundLocation(lat, lon, zoom, out x, out y);
    }
