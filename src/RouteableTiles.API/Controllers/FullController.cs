...
namespace RouteableTiles.API.Controllers
{
    ...
    public class FullController : ControllerBase
    {
        ...
        public object GetOsmXml(uint z, uint x, uint y)
        {
            ...
            return new RedirectToActionResult(nameof(GetOsmXmlVersion), "Full", routeValues);
        }
        
        [HttpGet("full/{timestamp}/{z}/{x}/{y}.osm")]
        public object GetOsmXmlVersion(string timestamp, uint z, uint x, uint y)
        {
            ...
            var data = dbOn.GetTile((x, y));

            var baseUrl = this.Request.HttpContext.Request.BasePath();
            if (!baseUrl.EndsWith("/")) baseUrl += "/";
            baseUrl = $"{baseUrl}full/{timestamp}/{z}/{x}/{y}.osm";
